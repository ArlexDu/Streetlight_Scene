...
namespace ParticlePlayground {
	...
	public class PlaygroundParticlesC : MonoBehaviour {
		...
		public static void ThreadedCalculations (PlaygroundParticlesC playgroundParticles) {
			...
			bool 		renderModeStretch = playgroundParticles.particleSystemRenderer2.renderMode==ParticleSystemRenderMode.Stretch;
				...
				for (int p = 0; p<playgroundParticles.particleCache.Length; p++) {
					...
					if (playgroundParticles.playgroundCache.rebirth[p]) {
						...
						if (playgroundParticles.playgroundCache.life[p]<=PlaygroundC.globalTime+playgroundParticles.lifetime) {
							...
							playgroundParticles.particleCache[p].size = playgroundParticles.playgroundCache.size[p];
							...
							for (m = 0; m<playgroundParticles.manipulators.Count; m++) {
								if (playgroundParticles.manipulators[m].transform!=null) {
									...
									CalculateManipulator(playgroundParticles, playgroundParticles.manipulators[m], p, t, playgroundParticles.playgroundCache.position[p], manipulatorPosition, manipulatorDistance, localSpace);
								}
							}
							...
							for (m = 0; m<PlaygroundC.reference.manipulators.Count; m++) {
								if (PlaygroundC.reference.manipulators[m].transform!=null) {
									...
									CalculateManipulator(playgroundParticles, PlaygroundC.reference.manipulators[m], p, t, playgroundParticles.playgroundCache.position[p], manipulatorPosition, manipulatorDistance, localSpace);
								}
							}
							...
							if (!playgroundParticles.onlySourcePositioning) {
								...
								if (playgroundParticles.playgroundCache.velocity[p]!=Vector3.zero) {
									...
									if (!playgroundParticles.onlySourcePositioning && renderModeStretch)
								...
								// Set particle velocity to be able to stretch towards movement
								if (renderModeStretch)
									playgroundParticles.particleCache[p].velocity = (playgroundParticles.playgroundCache.targetPosition[p]-playgroundParticles.playgroundCache.previousTargetPosition[p]);
		...
		public static void CalculateManipulator (PlaygroundParticlesC playgroundParticles, ManipulatorObjectC thisManipulator, int p, float t, Vector3 particlePosition, Vector3 manipulatorPosition, float manipulatorDistance, bool localSpace) {
			if (thisManipulator.enabled && thisManipulator.transform.available && thisManipulator.strength!=0 && thisManipulator.willAffect) {
				if (!playgroundParticles.onlySourcePositioning) {
					// Attractors
		...
		IEnumerator LoadRoutine (int loadPointer) {
			if (loadTransition && loadTransitionTime>0 && transitionAvailable) {
		...
		IEnumerator LoadTransition (int loadPointer) {
			...
			// Transition
			while (PlaygroundC.globalTime<transitionStartTime+(loadTransitionTime-.001f) && inTransition && !abortTransition && loadTransition && loadTransitionTime>0 && currentParticleCount==particleCount) {
				float currentTime = PlaygroundC.globalTime;
				PlaygroundC.RunAsync (()=>{
					float t = TransitionType ((currentTime-transitionStartTime)/loadTransitionTime);
		...
		float TransitionType (float t) {
			if (loadTransitionType==TRANSITIONTYPEC.Linear)
				return t;
			else if (loadTransitionType==TRANSITIONTYPEC.EaseIn)
				return Mathf.Lerp (0f, 1f, 1f-Mathf.Cos(t*Mathf.PI*.5f));
			else if (loadTransitionType==ParticlePlayground.TRANSITIONTYPEC.EaseOut)
		...
		void OnEnable () {
			...
			if (particleCache==null) {
				...
				if (!loadFromStart || loadFromStart && snapshots.Count==0 || loadFromStart && snapshots.Count>0 && snapshots[loadFrom].loadMode==1)
